...
STOP_SURPRISE_MODE="${STOP_SURPRISE_MODE:-all_selected}"
...
echo "[preflight] ACQUISITION_PROFILE=${ACQUISITION_PROFILE}"
echo "[preflight] SURPRISE_CLEANUP_QEDGE_PENALTY=${SURPRISE_CLEANUP_QEDGE_PENALTY}"
echo "[preflight] SURPRISE_CLEANUP_QEDGE_FLOOR=${SURPRISE_CLEANUP_QEDGE_FLOOR}"
echo "[preflight] SURPRISE_CLEANUP_RESPONSE_WEIGHT=${SURPRISE_CLEANUP_RESPONSE_WEIGHT}"
echo "[preflight] SURPRISE_CLEANUP_EXPLORE_SCALE=${SURPRISE_CLEANUP_EXPLORE_SCALE}"
...
echo "[preflight] OUTPUT_ROOT=${OUTPUT_ROOT}"
echo "[preflight] ORACLE_MODE=${ORACLE_MODE}"
echo "[preflight] INCREMENTAL_Q_EXPANSION_FLAG=${INCREMENTAL_Q_EXPANSION_FLAG:-N/A}"
echo "[preflight] ENABLE_BASIN_CLUSTERING_FLAG=${ENABLE_BASIN_CLUSTERING_FLAG:-N/A}"
echo "[preflight] ENABLE_SELECTIVE_REFINEMENT_FLAG=${ENABLE_SELECTIVE_REFINEMENT_FLAG:-N/A}"
echo "[preflight] MAX_REFINED_MINIMA=${MAX_REFINED_MINIMA}"
echo "[preflight] MAX_OPTIONAL_REFINED_BASINS=${MAX_OPTIONAL_REFINED_BASINS}"
echo "[preflight] MANDATORY_BASINS_CAN_EXCEED_CAP_FLAG=${MANDATORY_BASINS_CAN_EXCEED_CAP_FLAG}"
echo "[preflight] HIGH_RISK_OVERFLOW_POLICY=${HIGH_RISK_OVERFLOW_POLICY}"
echo "[preflight] STOP_MAX_ITERATIONS=${STOP_MAX_ITERATIONS}"
echo "[preflight] STOP_SURPRISE_MODE=${STOP_SURPRISE_MODE}"
echo "[preflight] TRUSTED_SURPRISE_MIN_DENOMINATOR=${TRUSTED_SURPRISE_MIN_DENOMINATOR}"
echo "[preflight] TRUSTED_SURPRISE_MIN_FRACTION=${TRUSTED_SURPRISE_MIN_FRACTION}"
echo "[preflight] TOPOLOGY_CLASSIFICATION_FLAG=${TOPOLOGY_CLASSIFICATION_FLAG:-N/A}"
echo "[preflight] TOPOLOGY_GAP_NK=${TOPOLOGY_GAP_NK}"
echo "[preflight] TOPOLOGY_GAP_BACKEND=${TOPOLOGY_GAP_BACKEND}"
echo "[preflight] TOPOLOGY_GAP_TOL_REL=${TOPOLOGY_GAP_TOL_REL}"
echo "[preflight] TOPOLOGY_GAP_TOL_ABS=${TOPOLOGY_GAP_TOL_ABS}"
echo "[preflight] TOPOLOGY_GAP_K_CHUNK=${TOPOLOGY_GAP_K_CHUNK}"
...
export SURPRISE_CLEANUP_QEDGE_PENALTY
export SURPRISE_CLEANUP_QEDGE_FLOOR
export SURPRISE_CLEANUP_RESPONSE_WEIGHT
export SURPRISE_CLEANUP_EXPLORE_SCALE
    ...
    if [ -n "${existing_exact_job}" ] && status_true "${iter_dir}" "exact_submitted"; then
      ...
      exact_job="$(
        sbatch "${exact_sbatch_args[@]}" \
          --export=ALL,PROJECT_DIR="${PROJECT_DIR}",PYTHON_BIN="${PYTHON_BIN}",CONDA_ENV="${CONDA_ENV}",CUDA_MODULE="${CUDA_MODULE}",OUTPUT_ROOT="${OUTPUT_ROOT}",RUN_ID="${RUN_ID}",ITER="${iter}",WORLD_SIZE="${WORLD_SIZE}",Q_EXPANSION_FLAG="${Q_EXPANSION_FLAG}",DELTA_REFINEMENT_FLAG="${DELTA_REFINEMENT_FLAG}",MAX_Q_REFINEMENTS="${MAX_Q_REFINEMENTS}",MAX_DELTA_REFINEMENTS="${MAX_DELTA_REFINEMENTS}",N_DELTA_REFINED="${N_DELTA_REFINED}",POSITIVE_DELTA_GAP_TOL="${POSITIVE_DELTA_GAP_TOL}",ORACLE_MODE="${ORACLE_MODE}",INCREMENTAL_Q_EXPANSION_FLAG="${INCREMENTAL_Q_EXPANSION_FLAG}",LOCAL_BOX_INSTRUMENTATION_FLAG="${LOCAL_BOX_INSTRUMENTATION_FLAG}",ENABLE_BASIN_CLUSTERING_FLAG="${ENABLE_BASIN_CLUSTERING_FLAG}",ENABLE_SELECTIVE_REFINEMENT_FLAG="${ENABLE_SELECTIVE_REFINEMENT_FLAG}",MAX_REFINED_MINIMA="${MAX_REFINED_MINIMA}",MAX_OPTIONAL_REFINED_BASINS="${MAX_OPTIONAL_REFINED_BASINS}",MANDATORY_BASINS_CAN_EXCEED_CAP_FLAG="${MANDATORY_BASINS_CAN_EXCEED_CAP_FLAG}",HIGH_RISK_OVERFLOW_POLICY="${HIGH_RISK_OVERFLOW_POLICY}",MAX_EDGE_RISK_BASINS="${MAX_EDGE_RISK_BASINS}",MAX_DELTA_NEAR_EPS_BASINS="${MAX_DELTA_NEAR_EPS_BASINS}",MAX_NEAR_DEGENERATE_BASINS="${MAX_NEAR_DEGENERATE_BASINS}",ENERGY_WINDOW_PRUNING_FLAG="${ENERGY_WINDOW_PRUNING_FLAG}",TOPOLOGY_CLASSIFICATION_FLAG="${TOPOLOGY_CLASSIFICATION_FLAG}",TOPOLOGY_GAP_NK="${TOPOLOGY_GAP_NK}",TOPOLOGY_GAP_BACKEND="${TOPOLOGY_GAP_BACKEND}",TOPOLOGY_GAP_TOL_REL="${TOPOLOGY_GAP_TOL_REL}",TOPOLOGY_GAP_TOL_ABS="${TOPOLOGY_GAP_TOL_ABS}",TOPOLOGY_GAP_K_CHUNK="${TOPOLOGY_GAP_K_CHUNK}" \
  ...
  if [ "${ENABLE_STOP_CONTROLLER}" = "1" ]; then
    if status_true "${iter_dir}" "stop_checked" && [ -f "${stop_metrics}" ]; then
      ...
      stop_args=(
        ...
        --stop-surprise-mode "${STOP_SURPRISE_MODE}"
        --trusted-surprise-min-denominator "${TRUSTED_SURPRISE_MIN_DENOMINATOR}"
        --trusted-surprise-min-fraction "${TRUSTED_SURPRISE_MIN_FRACTION}"
...
report_status="${OUTPUT_ROOT}/reports/report_generation_status.json"
mkdir -p "${OUTPUT_ROOT}/reports"
if "${PYTHON_BIN}" -m ml_phase.report_builder \
  --run-id "${RUN_ID}" \
  --run-root "${OUTPUT_ROOT}/active_runs" \
  --output "${OUTPUT_ROOT}/reports/active_learning_phase_boundary_report.tex"; then
  if command -v pdflatex >/dev/null 2>&1; then
    if pdflatex -interaction=nonstopmode -halt-on-error \
      -output-directory "${OUTPUT_ROOT}/reports" \
      "${OUTPUT_ROOT}/reports/active_learning_phase_boundary_report.tex"; then
      cat > "${report_status}" <<EOF
{"report_generated": true, "pdf_generated": true, "status": "ok"}
EOF
    else
      echo "[warn] pdflatex failed; numerical run outputs remain complete." >&2
      cat > "${report_status}" <<EOF
{"report_generated": true, "pdf_generated": false, "status": "pdflatex_failed"}
EOF
    fi
  else
    echo "[warn] pdflatex not found; TeX report generated only."
    cat > "${report_status}" <<EOF
{"report_generated": true, "pdf_generated": false, "status": "pdflatex_missing"}
EOF
  fi
else
  echo "[warn] final report generation failed; numerical run outputs remain complete." >&2
  cat > "${report_status}" <<EOF
{"report_generated": false, "pdf_generated": false, "status": "report_builder_failed"}
EOF
fi
