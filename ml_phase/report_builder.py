...
_DEFAULT_TEMPLATE_RELATIVE = Path("report/active_learning_phase_boundary_report.tex")

_BUILTIN_REPORT_TEMPLATE = r"""
...
\section{Run Summary}
\textbf{Run ID:} {{RUN_ID}}\\
\textbf{Run mode:} {{RUN_MODE}}\\
\textbf{Candidate domain mode:} {{CANDIDATE_DOMAIN_MODE}}\\
\textbf{Selection mode:} {{SELECTION_MODE}}\\
\textbf{Initialization:} {{INITIALIZATION}}\\
\textbf{Completed active-learning iterations:} {{COMPLETED_ITERATIONS}}\\
\textbf{Final dataset:} {{FINAL_DATASET}}\\
\textbf{Current exact samples:} {{N_EXACT}}\\
\textbf{Latest dataset phase counts:} normal {{N_NORMAL}}, uniform\_SC {{N_UNIFORM_SC}}, FFLO {{N_FFLO}}\\
\textbf{Latest append status:} {{LATEST_APPEND_STATUS}}

\section{Numerical Status}
\textbf{Merged exact points:} {{N_MERGED_EXACT}}\\
\textbf{Trusted exact points:} {{N_TRUSTED_EXACT}}\\
\textbf{Training-eligible exact points:} {{N_TRAINING_ELIGIBLE_EXACT}}\\
\textbf{Rerun-required points:} {{N_RERUN_REQUIRED}}\\
\textbf{q-expanded count:} {{Q_EXPANDED_COUNT}}\\
\textbf{q-unresolved count:} {{Q_UNRESOLVED_COUNT}}\\
\textbf{Delta-refined count:} {{DELTA_REFINED_COUNT}}\\
\textbf{Delta-unresolved count:} {{DELTA_UNRESOLVED_COUNT}}

\section{Stop Status}
\textbf{Stop status:} {{STOP_STATUS}}\\
\textbf{Stop reason:} {{STOP_REASON}}\\
\textbf{Stop conditions:} {{STOP_CONDITIONS}}
...
\toprule
iter & selected & merged & training & rerun & dataset \\
\midrule
...
The project template was unavailable at runtime, so this built-in fallback
template was used. Numerical outputs are unchanged; regenerate the full report
after restoring \texttt{report/active\_learning\_phase\_boundary\_report.tex}
if richer figures and diagnostics are needed.
\end{document}
"""
...
def _fmt_condition_summary(raw: dict | None) -> str:
    ...
    labels = {
        ...
        "C4_label_surprise_rate": "selected surprise gate",
...
def _fmt_stop_metric_summary(raw: dict | None) -> str:
    ...
    labels = {
        ...
        "label_surprise_all_selected": "all-selected surprise",
        "label_surprise_trusted": "trusted surprise",
        "label_surprise_hard_risk": "hard-risk surprise",
        "label_surprise_selected_for_gate": "selected surprise gate",
    ...
    surprise_details = raw.get("surprise_details") if isinstance(raw.get("surprise_details"), dict) else {}
    if surprise_details:
        parts.append(f"surprise mode: {_fmt(surprise_details.get('stop_surprise_mode'))}")
        parts.append(
            "trusted denominator valid: "
            + _fmt_bool(surprise_details.get("trusted_surprise_denominator_valid"))
        )
    if "numerical_frontier_status" in raw:
        parts.append(f"numerical frontier: {_fmt(raw.get('numerical_frontier_status'))}")
    if "publication_ready" in raw:
        parts.append(f"publication ready: {_fmt_bool(raw.get('publication_ready'))}")
...
def _fmt_acq_weight_summary(cfg_raw: dict) -> str:
    ...
    profile = str(cfg_raw.get("acquisition_profile", "full"))
    if profile == "simple_phase":
        w_ext_simple = _fmt(cfg_raw.get("w_ext_simple_start"))
        if str(cfg_raw.get("w_ext_simple_schedule", "constant")) == "piecewise":
            w_ext_simple = (
                f"{_fmt(cfg_raw.get('w_ext_simple_start'))}"
                f"\\rightarrow{_fmt(cfg_raw.get('w_ext_simple_mid'))}"
                f"\\rightarrow{_fmt(cfg_raw.get('w_ext_simple_end'))}"
            )
        return (
            r"\begin{aligned}"
            + r"A_{0,\mathrm{main}} &= A_{\mathrm{phase,simple}} + A_{\mathrm{explore,simple}} \\ "
            + f"w_{{\\mathrm{{cls}}}} &= {_fmt(cfg_raw.get('w_cls_simple'))}, "
            + f"w_{{\\mathrm{{NS}}}} &= {_fmt(cfg_raw.get('w_ns_simple'))}, "
            + f"w_{{\\mathrm{{UF}}}} &= {_fmt(cfg_raw.get('w_uf_simple'))}, "
            + f"w_g &= {_fmt(cfg_raw.get('w_grad_simple'))}, "
            + f"w_{{\\mathrm{{reg}}}} &= {_fmt(cfg_raw.get('w_reg_simple'))} \\\\ "
            + f"w_{{\\mathrm{{ext,simple}}}} &= {w_ext_simple}, "
...
def _read_report_template(template_path: Path) -> tuple[str, str]:
    candidates: list[Path] = []
    if template_path.is_absolute():
        candidates.append(template_path)
    else:
        candidates.append(Path.cwd() / template_path)
        candidates.append(Path(__file__).resolve().parents[1] / template_path)
    seen: set[Path] = set()
    ...
    if _template_key(template_path) == _template_key(_DEFAULT_TEMPLATE_RELATIVE):
        return _BUILTIN_REPORT_TEMPLATE, "builtin:fallback_active_learning_phase_boundary_report"
    searched = "\n".join(str(p) for p in candidates)
    raise FileNotFoundError(
        "LaTeX report template not found. Searched:\n"
        f"{searched}\n"
        "Pass --template with an existing file, or include "
        "report/active_learning_phase_boundary_report.tex in the package."
    )
...
def build_report(run_dir: Path, template_path: Path, output_tex: Path) -> Path:
    ...
    tex, template_source = _read_report_template(template_path)
    tex = "% Template source: " + template_source.replace("\\", "/") + "\n" + tex
...
def _parse_args() -> argparse.Namespace:
    ...
    p.add_argument(
        "--template",
        type=Path,
        default=_DEFAULT_TEMPLATE_RELATIVE,
