...
RUN_ID="${RUN_ID:-active_boundary_discovery_rankcap_k3_5iter_validation_v1}"
OUTPUT_ROOT="${OUTPUT_ROOT:-ML_Phase_512_RankCapK3_5Iter}"
...
echo "[submit] package=rankcap_k3_5iter_validation"
echo "[submit] run_id=${RUN_ID}"
echo "[submit] output_root=${OUTPUT_ROOT}"
echo "[submit] n_iters=${N_ITERS}"
echo "[submit] semantics=iter000 random seed + iter001-iter005 five acquisition-selected batches"
...
echo "[collect] build 5-iteration validation report"
...
echo "[done] return archive: ${OUTPUT_ROOT}/rankcap_k3_5iter_validation_results.tar.gz"
